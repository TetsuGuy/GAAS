...
export class GothController {
    ...
    async uploadPost(request: Request & MulterRequest, response: Response): Promise<void> {
        try {
            if (!request.file) {
            ...
            response.send(`File ${originalname} uploaded successfully. You can close this window now.`);
        } catch (e) {
    ...
    async generate(_request: Request, response: Response): Promise<void> {
        response.sendFile(path.join(rootDir, "svelte", "public", "index.html"));
    }
